...
public class CaptureManager {
    ...
    private final CameraPreview.StateListener stateListener = new CameraPreview.StateListener() {
        ...
        public void cameraError(Exception error) {
            displayFrameworkBugMessageAndExit(
                   "Ha ocurrido un error en la app."
            );
        }
    ...
    protected void returnResult(BarcodeResult rawResult) {
        // Obtén la imagen del código de barras si está habilitado
        String barcodeImagePath = getBarcodeImagePath(rawResult);

        // Crea el Intent para pasar los resultados
        Intent resultIntent = resultIntent(rawResult, barcodeImagePath);
        activity.setResult(Activity.RESULT_OK, resultIntent);



        // Obtén el texto del código QR
        String qrCodeData = rawResult.getText();

        // Verifica la condición para determinar qué actividad iniciar
        if (GlobalPermisos.getListaNombresCuentas() == null) {
            // Inicia MainActivity si la lista de nombres de cuentas es null
            // Cierra y finaliza la actividad
            closeAndFinish();
            Intent mainActivityIntent = new Intent(activity, MainActivity.class);
            mainActivityIntent.putExtra("qrCode", qrCodeData);
            activity.startActivity(mainActivityIntent);
        } else {
            // Inicia SupervisionActivity si la lista de nombres de cuentas no es null
            // Verifica si el listener está definido



            Intent supervisionActivityIntent = new Intent(activity, SupervisionActivity.class);
            supervisionActivityIntent.putExtra("qrCode", qrCodeData);
            activity.startActivity(supervisionActivityIntent);
        }
    }

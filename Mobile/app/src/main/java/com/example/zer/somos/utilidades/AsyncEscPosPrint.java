...
public abstract class AsyncEscPosPrint extends AsyncTask<AsyncEscPosPrinter, Integer, AsyncEscPosPrint.PrinterStatus> {
    ...
    protected void onProgressUpdate(Integer... progress) {
        switch (progress[0]) {
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.PROGRESS_CONNECTING:
                this.dialog.setMessage("Connecting printer...");
                break;
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.PROGRESS_CONNECTED:
                this.dialog.setMessage("Printer is connected...");
                break;
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.PROGRESS_PRINTING:
                this.dialog.setMessage("Printer is printing...");
                break;
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.PROGRESS_PRINTED:
    ...
    protected void onPostExecute(PrinterStatus result) {
        ...
        switch (result.getPrinterStatus()) {
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.FINISH_SUCCESS:
                new AlertDialog.Builder(context)
                        .setTitle("Success")
                        .setMessage("Congratulation ! The texts are printed !")
                        .show();
                break;
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.FINISH_NO_PRINTER:
                new AlertDialog.Builder(context)
                        .setTitle("No printer")
                        .setMessage("The application can't find any printer connected.")
                        .show();
                break;
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.FINISH_PRINTER_DISCONNECTED:
                new AlertDialog.Builder(context)
                    .setTitle("Broken connection")
                    .setMessage("Unable to connect the printer.")
                    .show();
                break;
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.FINISH_PARSER_ERROR:
                new AlertDialog.Builder(context)
                    .setTitle("Invalid formatted text")
                    .setMessage("It seems to be an invalid syntax problem.")
                    .show();
                break;
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.FINISH_ENCODING_ERROR:
                new AlertDialog.Builder(context)
                    .setTitle("Bad selected encoding")
                    .setMessage("The selected encoding character returning an error.")
                    .show();
                break;
            case com.example.zer.somos.utilidades.AsyncEscPosPrint.FINISH_BARCODE_ERROR:
                new AlertDialog.Builder(context)
        ...
        if(this.onPrintFinished != null) {
            if (result.getPrinterStatus() == com.example.zer.somos.utilidades.AsyncEscPosPrint.FINISH_SUCCESS) {
